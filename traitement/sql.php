<?php
  ...
  function get_articles_from_categories_and_titre_and_auteur($categories, $recherche) {
    require "config.php";
    $sql = "SELECT DISTINCT a.* FROM (SELECT a.* FROM article a JOIN user u ON a.id_user=u.id WHERE LOWER(a.titre) LIKE '%".trim(strtolower($recherche))."%' OR LOWER(u.pseudo) LIKE '%".trim(strtolower($recherche))."%') a LEFT JOIN lien_categorie_article l ON a.id=l.id_article";
